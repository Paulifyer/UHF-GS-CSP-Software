...
void csp_conn_get_header_values(uint8_t * packet1){ //self-defined function that is not in library
	...
	packet1[0] = conn->idin.dst;
	packet1[1] = conn->idin.src;
	packet1[2] = conn->idin.sport;
	packet1[3] = conn->idin.dport;
	packet1[4] = conn->idin.flags;
